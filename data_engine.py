...
def rebuild_ticker(symbol: str) -> ScrapeResult:
    """Delete a ticker's cached parquet and trigger a fresh full-history
    download. Use when a past stock split left cached prices unadjusted,
    or to recover from a corrupt cache for a single ticker."""
    p = _parquet_path(symbol)
    if p.exists():
        try:
            p.unlink()
            log.info("Deleted cached parquet for %s", symbol)
        except OSError as exc:
