...
_FINVIZ_COLS = [
    "symbol", "fiscal_period", "fiscal_end_date", "earnings_date",
    "eps_actual", "eps_estimate", "eps_reported_actual",
    "eps_reported_estimate", "sales_actual", "sales_estimate",
    "fetched_at", "run_id",
]
...
_SCHEMA_BY_SOURCE = {
    ...
    config.RAW_SOURCE_FINVIZ: _FINVIZ_COLS,
}
...
def new_run_id() -> str:
    ...
    ``{YYYYMMDDHHMMSS}_{uuid4 hex}`` so directory listings sort by name in
    approximate creation order, independent of mtime.

    Uses the FULL uuid4 hex (not a truncated prefix) so two runs started in the
    same second can't collide into one run file and silently merge captures.
    Parsers split on "_" and read the timestamp (first token), so the longer
    suffix is harmless.
    """
    stamp = datetime.now().strftime("%Y%m%d%H%M%S")
    return f"{stamp}_{uuid.uuid4().hex}"
...
def append_finviz_rows(
    rows: list[dict], run_id: str, fetched_at: Optional[datetime] = None,
) -> int:
    """Append finviz earningsData entries verbatim (both the adjusted
    and GAAP `*Reported*` EPS fields plus sales + dates) so a replay can
    reconstruct the consumer rows without re-scraping finviz."""
    return _append_rows(config.RAW_SOURCE_FINVIZ, rows, run_id, fetched_at)
