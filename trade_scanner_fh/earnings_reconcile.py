...
Multi-source priority chain (current ordering):

    last_earnings: nasdaq > yahoo > finviz > zacks > finnhub
    next_earnings: same chain. Finnhub future rows must NOT carry
                   report_date_proxy=True.

Position of the ordering vs prior versions:
  * Nasdaq + Yahoo (live calendar feeds) outrank zacks/finnhub.
    Calendar sources publish real announcement dates; history-derived
    dates lag actual announcements by anywhere from minutes to weeks.
  * Zacks history is the next fallback when calendar sources don't
    cover a ticker — its `report_date` is the real announcement.
  * Finnhub is the last resort — only used when nothing else covers
    the ticker.
  (SEC EDGAR source removed 2026-05-31.)
...
# Live calendar feeds (nasdaq, yahoo) outrank history-derived sources;
# among history-derived, finviz > zacks > finnhub (matching the
# earnings_history dedup priority). Finviz carries real announcement
# dates + times so it leads the history group.
_LAST_PRIORITY = ("nasdaq", "yahoo", "finviz", "zacks", "finnhub")
# Sources eligible for the next_earnings position, in priority order.
# Finnhub future rows must NOT be proxy=True.
_NEXT_PRIORITY = ("nasdaq", "yahoo", "finviz", "zacks", "finnhub")

# When the source label collapses to "{src}_derived" / "{src}":
_SAME_SRC_LABEL = {
    "finviz": "finviz_derived",
    "zacks": "zacks_derived",
...
_HISTORY_SOURCES = {"finviz", "zacks", "finnhub"}
...
def reconcile_earnings_dates(
    ...
    *,
    ...
    _dates_supplied = dates_df is not None
    if dates_df is None:
    ...
    # source="finviz" row, because the per-quarter history holds only PAST
    # quarters (forward/estimate rows are dropped at fill time). Merge the
    ...
    for t in set(fv_hist) | set(fv_dates):
        ...
        d_last, d_next = fv_dates.get(t, (pd.NaT, pd.NaT))
        last = h_last if pd.notna(h_last) else d_last
        # Prefer the cache's forward date for NEXT; fall back to any future
        # history row (normally none for finviz). _pick_next still applies
        # the > today filter, so a stale cached date can't leak through.
        nxt = d_next if pd.notna(d_next) else h_next
        finviz_lookup[t] = (last, nxt)
    ...
    all_known = (set(lookups["finviz"]) | set(lookups["zacks"])
                 | set(lookups["finnhub"]) | set(lookups["nasdaq"])
                 | set(lookups["yahoo"]))
    ...
    affected_set = None if affected_tickers is None else set(affected_tickers)

    # Merge under the shared cache lock: rows for processed tickers replace any
    # existing entries; rows for tickers OUTSIDE the candidate set are
    # preserved. When we loaded the cache from disk (production), re-read the
    # freshest snapshot inside the lock so rows a concurrent writer added for
    # non-candidate tickers since our load aren't dropped. When the caller
    # supplied dates_df (tests / in-memory), honor it verbatim.
    with ec.DATES_WRITE_LOCK:
        if _dates_supplied:
            base = dates_df
        else:
            latest = ec.load_earnings_cache()
            base = latest if latest is not None else dates_df

        # Which existing rows to drop before appending the recomputed ones:
        #  - targeted reconcile: the FULL affected set, so a targeted ticker
        #    that now yields NO usable date has its stale row REMOVED rather
        #    than silently preserved (audit M24);
        #  - full sweep: only the tickers we produced fresh rows for.
        if affected_set is not None:
            drop = affected_set
        elif new_df is not None:
            drop = set(new_df["ticker"])
        else:
            drop = set()

        had_base = base is not None and not base.empty
        if new_df is None:
            # No fresh rows. Only write if we're actually removing a stale row
            # for a targeted ticker that yielded nothing — otherwise leave the
            # cache untouched (preserves the empty-input / unknown-ticker
            # no-op behavior).
            if (not had_base or not drop
                    or not bool(base["ticker"].isin(drop).any())):
                log.debug("reconcile: nothing to write — earnings_dates untouched")
                return z_count, y_count, aug_count
            combined = base.loc[~base["ticker"].isin(drop)].reset_index(drop=True)
        elif had_base:
            keep = base.loc[~base["ticker"].isin(drop)]
