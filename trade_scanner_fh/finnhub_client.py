...
def _request(endpoint: str, params: dict, *, timeout: float = 15.0):
    ...
    try:
        r = requests.get(
            ...
            timeout=timeout,
            # The API token rides in the query string; Finnhub's REST API never
            # legitimately 3xx-redirects, so refuse redirects rather than
            # re-send the token-bearing URL to an attacker-supplied Location.
    ...
    # Cap the body before parsing so a hostile/MITM'd response can't force a
    # multi-hundred-MB buffer + JSON parse (audit M23). r.content is already
    # buffered by requests, so this is a ceiling on parse cost.
    body = getattr(r, "content", b"") or b""
    if len(body) > config.FINNHUB_MAX_RESPONSE_BYTES:
        log.warning("Finnhub %s response too large (%d bytes) — rejecting",
                    endpoint, len(body))
...
def fetch_earnings_dates(
    ...
    past_dates = []
    future_dates = []
    # Bound the iteration even though _request already caps the body size — a
    # symbol's calendar over the lookback window is ~tens of rows; 5000 is far
    # above any legitimate count (audit: list size was unbounded).
    for evt in events[:5000]:
        ...
        try:
            ...
            # OutOfBoundsDatetime subclasses ValueError — all caught here.
            continue
        if ts <= today_ts:
