...
def _flush_pending_to_disk(
    ...
    *,
    ...
    # concurrent Zacks/Finnhub fills serialize their merges
    # instead of overwriting each other's appended rows.
    with eh.HISTORY_WRITE_LOCK:
        ...
        # nano-caps): finnhub `actual` often carries pre-split/total-NI
...
def _finalize_fill(affected_tickers: list[str]) -> None:
    ...
    if not affected_tickers:
        return
    from . import earnings_history as eh
    # Serialize the read→recompute→write against concurrent fills — see the
    # matching lock in finviz_fill._finalize_fill / earnings_history.
    with eh.HISTORY_WRITE_LOCK:
        existing = eh.load_earnings_history()
        if existing is not None and not existing.empty:
