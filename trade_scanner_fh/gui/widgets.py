...
class LogPanel(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Bound the in-memory document: this is an always-on app and bulk
        # scrapes run for hours emitting per-ticker lines, so an uncapped
        # QTextEdit grows monotonically and each append relayouts an ever-
        # larger document. The full log is still persisted to disk, so the
        # widget only needs a rolling tail. Oldest lines self-trim past this.
        self.text.document().setMaximumBlockCount(10000)
    ...
    def _append(self, msg: str):
        # Only re-pin to the bottom if the user was already at the bottom —
        # otherwise scrolling up to read history gets yanked back down on the
        # next log line (which, during a multi-hour fill, is constant).
