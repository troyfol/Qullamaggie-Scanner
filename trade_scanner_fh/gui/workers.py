...
class ScanWorker(QThread):
    ...
    def __init__(
        ...
        # When True, tickers that pass an earlier period are stripped
        # from the symbol list before any later period scans. Caller
        # is responsible for ordering params_list so the "first claim"
        # period appears first (smallest timeframe / most proximal
        # chunk). Off by default — full universe scanned for every
        # period, matching legacy behavior.
        self.omit_intra_run = omit_intra_run
    ...
    def run(self):
        ...
        # Mutable working set for the intra-run omit filter. When
        # omit_intra_run is False, this stays equal to self.symbols and
        # every period scans the full universe (legacy behavior). When
        # True, tickers that hit in an earlier period are stripped before
        # the next period dispatches, so each ticker can only show up in
        ...
        try:
            for idx, (label, params) in enumerate(self.params_list, 1):
                ...
                if self.omit_intra_run and idx > 1:
                    stripped = len(self.symbols) - len(working_symbols)
                    if stripped > 0:
                        self.log_msg.emit(
                            f"  Intra-run filter: scanning "
                            f"{len(working_symbols)} of {len(self.symbols)} "
                            f"tickers ({stripped} hit earlier period(s))"
                        )
                    if not working_symbols:
                        self.log_msg.emit(
                            "  All tickers were claimed by earlier "
                            "periods — skipping this timeframe."
                        )
                ...
                # Intra-run omission: strip this period's hits from the
                # working set so they cannot reappear in any later period.
                if (self.omit_intra_run and not df.empty
                        and "symbol" in df.columns):
                    hits = set(df["symbol"].tolist())
                    if hits:
                        working_symbols = [
                            s for s in working_symbols if s not in hits
                        ]
...
class ZacksFillWorker(QThread):
    ...
    def _on_imperva_block(self, count: int, session) -> str:
        ...
        """
        try:
            # Honor a stop that was requested as this block trips (race:
            # request_stop() set _stop + decision='stop' + the event, but if we
            # then cleared the event and reset decision to 'continue' we'd drop
            # the stop AND block forever on wait()). Short-circuit instead.
            if self._stop[0]:
                return "stop"
            self.log_msg.emit(
    ...
    def run(self):
        ...
        try:
            ...
            if self.mode == "bulk":
                ...
                # caller-supplied candidate list. gap_fill_finnhub already
                # iterates only the provided symbols (no checkpoint resume),
                # so it's the right primitive for both.
                _label = ("Finnhub smart refresh" if self.mode == "targeted"
                          else "Finnhub gap fill")
                self.log_msg.emit(
                    f"{_label}: {len(self.symbols)} ticker(s)"
                )
...
class FinvizFillWorker(QThread):
    """Runs ``finviz_fill.bulk_fill_finviz`` / ``gap_fill_finviz`` on a
    background thread. Mirrors the FinnhubFillWorker control surface
    (start/stop/log/progress) and exposes an ``empty_identified`` signal
    so the GUI can grow the finviz skip list live as tickers finviz
    doesn't cover (no earningsData) are encountered.

    Finviz is paced deliberately slow (config.FINVIZ_MIN_INTERVAL_SEC ±
    jitter) for an overnight-safe bulk. On a block trigger (≥
    FINVIZ_CONSEC_BLOCK_LIMIT consecutive real failures) the worker
    pauses with exponential backoff then rewinds to the first ticker in
    the failure window. After FINVIZ_MAX_BLOCKS_PER_RUN total blocks it
    ...
    def run(self):
        ...
        try:
            ...
            def empty_cb(sym):
                try:
                    self.empty_identified.emit(sym)
                except Exception:
                    pass

            def block_cb(consec, blocks_so_far):
                self.log_msg.emit(
                    f"Finviz: hit {blocks_so_far} block-pause(s) — halting."
                )
                return "stop"
            ...
            if self.mode == "bulk":
                self.log_msg.emit(
                    ...
                    f"(slow overnight pace — may run many hours)"
                )
                ...
                # "targeted" == smart-refresh: process exactly the
                # caller-supplied candidate list. gap_fill_finviz already
                # iterates only the provided symbols (no checkpoint resume),
                # so it's the right primitive for both.
                _label = ("Finviz smart refresh" if self.mode == "targeted"
                          else "Finviz gap fill")
                self.log_msg.emit(
                    f"{_label}: {len(self.symbols)} ticker(s)"
                )
                filled, errors = finviz_fill.gap_fill_finviz(
                    self.symbols, self.blacklist,
                    progress_cb=progress_cb,
                    stop_flag=self._stop,
                    flush_every=self.flush_every,
...
# (EDGAR fill worker removed 2026-05-31 — earnings EDGAR source dropped.)
...
class BridgeWorker(QThread):
    ...
    def run(self):
        # Guard like ScanWorker: a pyautogui FailSafeException (the documented
        # corner-move abort) or any other error escaping start() would skip the
        # bridge's on_done callback, so _on_bridge_done never fires and the
        # Scan/Send/Manual buttons stay disabled forever (and an uncaught
        # exception out of QThread.run() can abort the frozen process). Emit
        # done(0,0) so the GUI re-enables its controls.
        try:
            self.bridge.start()
        except Exception as exc:
            log.error("BridgeWorker crashed: %s", exc, exc_info=True)
            self.log_msg.emit(f"Send aborted: {exc}")
            self.done.emit(0, 0)
...
class FirefoxCookieWaitWorker(QThread):
    ...
    def __init__(
        ...
        *,
        ...
        # Event (not a bare bool): lets request_stop() wake the poll wait
        # IMMEDIATELY instead of after up to a full poll interval, so app
        # close / cancel doesn't sit through a 1.5s sleep (matches the other
        # workers' stop primitives and tightens the closeEvent wait).
        self._stop = threading.Event()
    ...
    def run(self):
        ...
        while not self._stop.is_set() and time.monotonic() < deadline:
            ...
            # Interruptible wait — wakes instantly on request_stop().
