...
def test_default_email_is_placeholder():
    """Public-tree invariant: SEC_CONTACT_DEFAULT is a non-functional
    placeholder so no real contact email ever lives in source. The exact
    string is pinned because sec_contact_is_configured() compares against
    this literal sentinel — any drift would make the app send a fake email
    to SEC. Real emails come only from the gitignored sec_contact.txt or
    the SEC_CONTACT_EMAIL env var."""
    assert config.SEC_CONTACT_DEFAULT == "your.email@example.com"


def test_sec_contact_defaults_to_placeholder(_sec_tmp):
    """With no file and no env var, the resolver returns the placeholder
    default and reports the source as NOT configured, keeping the SEC
    universe source dormant until the user supplies a real email."""
    assert config.get_sec_contact_email() == config.SEC_CONTACT_DEFAULT
...
def test_clear_sec_contact_email(_sec_tmp):
    ...
    # Clearing the override falls back to the baked-in default — the
    # non-functional @example.com placeholder (configured=False), so the
    # SEC source goes dormant until a real email is supplied again.
    assert config.get_sec_contact_email() == config.SEC_CONTACT_DEFAULT
