...
def test_rebuild_ticker_swaps_in_fresh_download(tmp_path, monkeypatch):
    """rebuild_ticker moves the old parquet aside and does a full re-download.
    On success the fresh file is in place and no .rebuild_bak residue remains.
    (Audit M4: download-then-swap replaced the old delete-then-download order.)"""
    monkeypatch.setattr(data_engine.config, "PARQUET_DIR", tmp_path)
    clear_ohlcv_cache()
    ...
    def fake_download(sym):
        # Simulate a real successful download: write a fresh file.
        called["count"] += 1
        called["sym"] = sym
        pd.DataFrame({"Close": [2.0, 3.0]},
    ...
    assert pq.exists(), "fresh parquet should be in place after a successful rebuild"
    assert not (tmp_path / "OLD.parquet.rebuild_bak").exists(), "backup must be dropped"
    assert called["count"] == 1 and called["sym"] == "OLD"
    assert res.status == "ok" and res.rows_received == 42
    # The fresh download replaced the original content.
    assert list(pd.read_parquet(pq)["Close"]) == [2.0, 3.0]


def test_rebuild_ticker_preserves_cache_on_failed_download(tmp_path, monkeypatch):
    """Audit M4: a failed rebuild (download writes nothing) must RESTORE the
    original cache rather than leave the ticker with no history."""
    monkeypatch.setattr(data_engine.config, "PARQUET_DIR", tmp_path)
    clear_ohlcv_cache()
    ...
    def fake_failed_download(sym):
        # Real download_one returns without writing on error/no_data.
        return ScrapeResult(symbol=sym, status="error", rows_received=0)

    with patch("trade_scanner_fh.data_engine.download_one",
               side_effect=fake_failed_download):
        res = data_engine.rebuild_ticker("OLD")

    assert pq.exists(), "original cache must be restored after a failed rebuild"
    assert not (tmp_path / "OLD.parquet.rebuild_bak").exists()
    assert list(pd.read_parquet(pq)["Close"]) == [1.0], "original content preserved"
