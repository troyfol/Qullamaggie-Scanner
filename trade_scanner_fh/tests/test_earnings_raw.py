...
def test_new_run_id_format():
    ...
    # Format: YYYYMMDDHHMMSS_<uuid4 hex>. The suffix is the FULL uuid4 hex
    # (32 chars) — audit fix so two same-second runs can't collide.
    assert len(rid) == 14 + 1 + 32
    assert rid[14] == "_"
...
def test_prune_walks_every_source(tmp_raw):
    ...
    for src in config.RAW_SOURCES:
        ...
        elif src == config.RAW_SOURCE_FINVIZ:
            earnings_raw.append_finviz_rows(
                [{"symbol": "X", "fiscal_period": "2025Q4",
                  "fiscal_end_date": "2025-12-31",
                  "earnings_date": "2026-02-01T16:30:00",
                  "eps_actual": 1.1, "eps_estimate": 1.0,
                  "eps_reported_actual": 0.9, "eps_reported_estimate": 0.8,
                  "sales_actual": 100.0, "sales_estimate": 95.0}], rid)
        else:
