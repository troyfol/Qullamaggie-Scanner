...
def test_spot_fill_writes_one_row(tmp_world, monkeypatch):
    # spot_fill_yahoo computes `today` from the real clock (it takes no
    # today param), so the mock dates must be relative to today — one
    # genuinely past, one genuinely future — or the test drifts as the
    # calendar advances. Wide (±45d) margins keep the past/future split
    # stable even across a midnight boundary mid-test.
    today = pd.Timestamp.today().normalize()
    past = today - pd.Timedelta(days=45)
    future = today + pd.Timedelta(days=45)
    _mock_yfinance(monkeypatch, [past, future])
    ...
    assert aapl["last_earnings"] == past
